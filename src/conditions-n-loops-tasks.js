...
function convertToRomanNumerals(num) {
  ...
  const arabicSymbols = [10, 9, 5, 4, 1];

  for (let i = 0; i < arabicSymbols.length; i += 1) {
...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const rightSumm = summ - leftSumm - arr[i];
...
function getNearestBigger(number) {
  ...
  let swap = -1;
  for (let i = n - 1; i > point; i -= 1) {
    if (digits[i] > digits[point]) {
      swap = i;
      break;
    }
  }

  [digits[point], digits[swap]] = [digits[swap], digits[point]];
